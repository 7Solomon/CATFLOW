...
export const LoadProjectModal = ({ isOpen, onClose, onLoad }: LoadProjectModalProps) => {
    ...
    const loadList = async () => {
        ...
        try {
            ...
            setError(t('modals.loadProject.loadError'));
        } finally {
            setLoading(false);
        }
    };
                ...
                <div className="p-2 max-h-[60vh] overflow-y-auto">
                    {loading ? (
                        ...
                        <div className="p-4 text-center text-red-400 bg-red-900/10 rounded m-2">
                            {error}
                            <button onClick={loadList} className="block mx-auto mt-2 text-sm text-indigo-400 hover:underline">{t('common.retry')}</button>
                        </div>
                    ) : projects.length === 0 ? (
                        <div className="p-8 text-center text-slate-500">
                            {t('modals.loadProject.noProjects')}
                        </div>
                    ) : (
